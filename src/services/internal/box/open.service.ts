...
export default class OpenBoxService {
  ...
  async open(redeemerAddress: string, id: string): Promise<any> {
    ...
    if (item.openDate && isAfterNow(item.openDate)) {
      throw new HttpException(400, `You cannot open the box before ${new Date(item.openDate).toUTCString()}`, 'INVALID_BOX_OPEN_DATE');
    }
    ...
    if (item.isNanoBox) {
      return {
        hasSignexTx: true,
        data: await this.getSignedTxBoxService.generateSignedTxOpenNanoBox(redeemerAddress, id)
      };
    }
