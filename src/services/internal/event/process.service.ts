...
import DepositItemFromNFTService from '@/services/external/box/depositItemFromNFT.service';
import MintFromGameService from '@/services/external/box/mintFromGame.service';
import OpenBoxService from "@/services/external/box/open.service";
import OpenNanoBoxService from '@/services/external/box/openNanoBox.service';
...
import TransferNFTService from "@/services/external/nft/transfer.service";
...
export default class ProcessEventService {
  ...
  private transferNFTService = new TransferNFTService();
  private mintFromGameService = new MintFromGameService();
  private openNanoBoxGameService = new OpenNanoBoxService();
  private depositItemFromNFTGameService = new DepositItemFromNFTService();
  private saleIDOService = new saleIDOService();
  ...
  async execute(event: EventInstance) {
    switch (event.name) {
      ...
      case EventName.Transfer:
        await this.transferNFTService.process(event);
        break;

      case EventName.MintFromGame:
        await this.mintFromGameService.process(event);
        break;

      case EventName.OpenNanoBox:
        await this.openNanoBoxGameService.process(event);
        break;

      case EventName.DepositItemFromNFT:
        await this.depositItemFromNFTGameService.process(event);
        break;

      case EventName.BuyIDO:
        await this.saleIDOService.process(event);
        break;

      case EventName.ClaimIDO:
        await this.saleIDOService.claimIDO(event);
        break;
    }
  }
  ...
  async handleRedeemEvent(event: EventInstance) {
    ...
    if (itemType.toUpperCase() == "BOX") {
      await this.createBoxService.create(
        userAddress,
        id,
        tokenId.toNumber(),
        txHash
      ...
      await this.createNftService.create(
        userAddress,
        id,
        extraType,
        tokenId.toNumber(),
  ...
  async handleOpenStarterBoxEvent(event: EventInstance) {
    ...
    const tokenIds = _.map(_.get(event.param, "tokenIds"), (item) =>
      BigNumber.from(item).toNumber()
    ).slice(1);
  ...
  async queryMissingEvents(
    ...
    if (contractName) {
      ...
      const missingEvents = await contract.queryFilter(
        {},
        fromBlock + 1,
        toBlock
